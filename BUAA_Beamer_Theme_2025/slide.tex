...
%感谢THU，PKU，RUC, BNU, FDU, SJTU各位前辈开发者在此次魔(jie)改(jian)中的启发与借鉴
%Update on 11/16/2025
%Modified By SHR
...
%overleaf楷体设置。overleaf支持的中文字体详见：https://cn.overleaf.com/learn/latex/Questions/Which_OTF_or_TTF_fonts_are_supported_via_fontspec%3F#Chinese
%\usepackage{ctex}
\usepackage{hyperref, caption}
\usepackage[T1]{fontenc}
\usepackage{setspace} % 1. 加载 setspace 宏包
\setstretch{1.2}
% other packages
...
% 1. 标题 (你的页脚用的是短版本)
\title[BUAA Beamer Theme]{BUAA Beamer Theme}
\subtitle{——Adapted from THU Beamer Theme}

% 2. 作者 (你的页脚用的是短版本)
\author[Civin.Wang]{Civin.Wang
}
...
\begin{document}
    ...
    \begin{frame}
        ...
        \begin{figure}[htpb]
            \begin{center}
                \includegraphics[width=0.4\linewidth]{BUAA-logo.png}
            \end{center}
        \end{figure}
    \end{frame}
...
\end{lstlisting}
    ...
    \begin{frame}{作图}
        ...
        \begin{figure}[tbph]
            \centering
            \includegraphics[width=0.4\linewidth]{BUAA-logo}
            \caption{这个校徽就是矢量图}
            \label{fig:buaa-logo}
        \end{figure}
    \end{frame}
